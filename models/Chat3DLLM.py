...
# ========= Object Agent =========
class ObjectAgent(nn.Module):
    def __init__(self, feat_dim, img_dim, loc_dim, hidden_dim):
        super().__init__()
        input_dim = feat_dim + img_dim + loc_dim
        self.input_proj = nn.Linear(input_dim, hidden_dim)
        encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=4, batch_first=True)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=2)
    def forward(self, feat, img_feat, locs):
        x = torch.cat([feat, img_feat, locs], dim=-1)
        x = self.input_proj(x)
        x = self.encoder(x)
        return x
...
class SpatialGraphAttentionLayer(nn.Module):
    def __init__(self, dim, heads=4, dropout=0.1, spatial_dim=6):
        super().__init__()
        self.heads = heads
        self.scale = (dim // heads) ** -0.5
        self.qkv = nn.Linear(dim, dim * 3)
        self.pos_proj = nn.Linear(spatial_dim, heads)
        self.proj = nn.Linear(dim, dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, obj_locs, mask=None):
        B, N, D = x.shape
        H = self.heads
        qkv = self.qkv(x)
        ...
        delta = obj_locs.unsqueeze(2) - obj_locs.unsqueeze(1)
        spatial_bias = self.pos_proj(delta)
...
class SpatialGraphTransformer(nn.Module):
    def __init__(self, hidden_dim, spatial_dim=6, depth=2, heads=4, dropout=0.1):
        super().__init__()
        self.layers = nn.ModuleList([
            nn.Sequential(
                SpatialGraphAttentionLayer(hidden_dim, heads, dropout, spatial_dim),
    ...
    def forward(self, obj_features, obj_locs, mask=None):
        x = obj_features
        for layer in self.layers:
            attn = layer[0](x, obj_locs, mask)
            x = x + attn
            x = layer[1](x)
            x = layer[2](x)
            x = layer[3](x)
...
class SceneAgent(SpatialGraphTransformer):
    def __init__(self, hidden_dim, spatial_dim=6, depth=2, heads=4):
        super().__init__(hidden_dim, spatial_dim, depth, heads)


class SceneAgentImg(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=4, batch_first=True)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=4)
    def forward(self, obj_features, mask=None):
        if mask is not None:
            x = self.encoder(obj_features, src_key_padding_mask=~mask)
        else:
            x = self.encoder(obj_features)
        return x 
...
class Chat3DLLM(nn.Module):
    ...
    def __init__(self, config):
        ...
        if not self.debug:
            ...
            #######################################################
            reasoning_token = ["<CoT>", "</CoT>", "<Answer>", "</Answer>", "<SI>"]
            self.llama_tokenizer.add_tokens(reasoning_token, special_tokens=True)
        ...
        # --- Agents
        self.object_agent = ObjectAgent(self.input_dim, self.img_input_dim, self.pos_dim, self.llama_dim)
        self.scene_agent = SceneAgent(self.input_dim)
        self.scene_agent_img = SceneAgentImg(self.input_dim)
        # --- Agents
        
        self.object_proj = nn.Sequential(
            nn.Linear(self.input_dim + self.input_dim, self.llama_dim),
            nn.GELU(),
        ...
        #############
        self.scene_img_proj = nn.Sequential(
            nn.Linear(self.img_input_dim, self.img_input_dim),
            nn.GELU(),
    ...
    def aggregate_scene(self, obj_embeds, scene_locs, scene_mask):
        scene_embeds = self.scene_agent(obj_embeds, scene_locs, mask=scene_mask)
        return scene_embeds

    def aggregate_scene_object_img(self, obj_img_embeds, scene_mask):
        scene_img_embeds = self.scene_agent_img(obj_img_embeds, mask=scene_mask)
        return scene_img_embeds
    ...
    def forward_train(self, scene_feat, scene_img_feat, scene_locs, scene_mask, obj_ids, assigned_ids, questions, answers, is_eval=False, **kwargs):
        object_embed, object_img_embed = self.encode_object_feat(scene_feat, scene_img_feat, scene_locs) # [bs, N_obj, dim]

        object_embed_refined = self.aggregate_scene(object_embed, scene_locs, scene_mask)
        object_img_embed_refined = self.aggregate_scene_object_img(object_img_embed, scene_mask)

        object_embed = torch.cat([object_embed, object_embed_refined], dim=-1)
        object_img_embed = torch.cat([object_img_embed, object_img_embed_refined], dim=-1)
    ...
    def evaluate(self, scene_feat, scene_img_feat, scene_locs, scene_mask, custom_prompt, obj_ids, assigned_ids, is_eval=True, **kwargs):
        ...
        for i in range(batch_size):
            ...
            answer_all_content = output_text
            ...
            # print(answer_all_content)

            output_texts.append(answer_content)
            output_all_texts.append(answer_all_content)

        return output_texts, output_all_texts
