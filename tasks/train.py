...
from models.Chat3DLLM import Chat3DLLM
...
def train(
    ...
    accum_iter = 1
    eval_freq = 6000  # len(train_loader)
...
def evaluate(
    ...
    for i, batch in tqdm(enumerate(val_loader)):
        for k in batch.keys():
            if type(batch[k]) == torch.Tensor:
                batch[k] = batch[k].to(device)
        with torch.no_grad():
            pred, pred_all = model(**batch, is_eval=True)
    ...
    if len(save_preds_CoT) > 0:
        save_preds_CoT = sorted(save_preds_CoT, key=lambda x: f"{x['scene_id']}_{x['gt_id']:03}_{x['qid']}")
        with open(os.path.join(config.output_dir, f"preds_epoch{epoch}_step{global_step}_rank{get_rank()}_{eval_name}_CoT.json"),
                  "w") as f:
            json.dump(save_preds_CoT, f, indent=4)
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'Chat3DLLM'))
    (
        model,
        model_without_ddp,
        optimizer,
