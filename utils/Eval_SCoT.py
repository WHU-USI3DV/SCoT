...
default_instance_attr_file = "./SCoT_Dataset/annotations/scannet_mask3d_val_attributes.pt"
...
def calc_scanrefer_score(preds, config=None):
    ...
    scannet_attribute_file = "./SCoT_Dataset/annotations/scannet_val_attributes.pt"
...
def calc_scan2cap_score(preds, tokenizer, scorers, config=None):
    ...
    gt_dict = json.load(open('./SCoT_Dataset/annotations/scan2cap_val_corpus.json'))
    tmp_preds_iou25 = {}
    tmp_preds_iou50 = {}
    tmp_targets = {}
    for pred in preds:
...
def calc_scanqa_score(preds, tokenizer, scorers, config=None):
    with open(f"./SCoT_Dataset/annotations/scanqa/ScanQA_v1.0_val.json", "r") as f:
        annos = json.load(f)
    outputs = []
    qset = set()
    val_scores = {}
...
def calc_scot_score(preds, tokenizer, scorers, config=None):
    val_scores = {}
    tmp_preds = {}
    tmp_targets = {}
    metrics = {
...
if __name__ == '__main__':
    ...
    # saved_preds = json.load(open('./output/ObjectAnalysis.json')) # change this

    saved_preds = json.load(open('./outputs/ValResults/SituatedPlanningOnlyAnswer.json'))
    ...
    val_scores = calc_scot_score(saved_preds, tokenizer=tokenizer, scorers=scorers)
    # val_scores = calc_scanrefer_score(saved_preds, tokenizer=tokenizer, scorers=scorers) # only for **Visual Grounding** and **Implicit Detection** evaluation
